...
- **캐릭터 이모티콘(제거됨)**: 작업용 사용에 불필요해 완전히 제거했습니다. 에이전트에게 더 이상 이모티콘을 요구하지 않고, 캐릭터 이미지 자산도 남아 있지 않습니다. 예전 대화에 `[[CODEPET_EMOTE:키]]` 표기가 저장되어 있어도 이미지로 바뀌지 않고 화면에서 조용히 걸러집니다.
...
에이전트가 로그나 파일 내용을 아주 많이 출력해도 그것만으로는 실행을 끊지 않습니다. 출력 관련 한도는 목적이 다른 세 가지로 분리되어 있습니다.

- **수집 한도** — 메모리 보호용입니다. 한도를 넘으면 출력의 앞부분과 최신 부분만 남기고 중간을 버리며, 프로세스는 계속 실행되어 최종 답변을 정상적으로 받습니다.
- **표시 한도** — 화면이 감당할 양입니다. 넘으면 "출력이 길어 일부 내용을 접었습니다."라고 알리고 접어서 보여줄 뿐, 실행은 그대로 이어집니다.
- **안전 상한(선택)** — 기본값은 없습니다. `settings.json`에 `agentOutputHardLimitMB`를 양수로 넣은 경우에만 동작하며, 이때 중단은 시간 초과나 프로바이더 오류와 구분해 표시됩니다.
...
~/.agora/
  ...
  sessions/<id>/run-logs/          # 실행 원본 출력 (최근 실행분만 보관)
  trash/                   # 삭제된 세션 (30일 보관)
```
...
- `src/chat/` — 에이전트 채팅방 코어(멘션 파싱, 그룹챗 프롬프트, 실행 인자 생성, CLI 실행, 이벤트 정규화, 세션 저장소, 첨부, 룸 오케스트레이션). `chat-window.js`/`chat-ipc.js` 외에는 Electron 비의존
