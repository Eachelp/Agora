...
- **전문 실행 (PLAN ⇄ ACT)** — 일반 대화나 토론에서 정리된 작업을 기획(PLAN) → 구현·검수·기록(ACT) 흐름으로 실행합니다.
  - `PLAN`: Planner와 Plan Reviewer가 작업 지시서(TASK.md)를 완성하고 사용자 승인을 기다립니다. 프로젝트 설정에서 기획 검수 담당자를 따로 지정할 수 있고, 비우면 구현 Reviewer를 재사용합니다.
  - `실행`: 승인된 작업 계약(Frozen Task)을 기준으로 Builder가 구현하고 독립 Reviewer가 검수하며 Recorder가 기록합니다.
  - `전체 실행`: 사용자의 사전 승인을 받아 기획 검수 통과 시 자동으로 구현·검수·기록까지 이어갑니다.
...
전문 실행 중 앱이 종료되면 에이전트를 자동 재개하지 않고 해당 Run을 중단 상태로 엽니다. Git checkpoint가 남아 있으면 변경 유지·작업 전 복원·폐기 중에서 선택합니다. Git이 아닌 워크스페이스는 현재 파일을 읽어 검수할 수 있지만 변경 diff가 없어 PASS를 자동 완료하지 않고 사용자 확인을 기다립니다.
