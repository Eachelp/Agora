...
> 상태: **최초 제안 기록 — 구현 범위와 차이는 아래 후속 안내 참조**
>
> 작성일: 2026-08-30
>
...
> 후속 안내(2026-09-06): 이 제안 뒤 자유토론 예산, 구조화 Preset, Journal, 역할·팀 상담, `@팀 실행`, 허용된 Handoff 검증과 선택적 Archivist가 구현되었다. 실제 팀 진입점은 제안 본문의 `@모두`가 아니라 **`@팀` / `@팀 실행`**이며, `@모두`는 일반 참가자 호출을 유지한다. 호출 순서의 권위는 기존 FSM에 있고, Role-driven routing·Professional V2·Orchestrator는 구현되지 않았다. 아래 제안 원문은 결정 과정 보존을 위해 유지한다. 현재 도달 범위는 [구현 계획의 후속 결정](AGORA_V1_5_IMPLEMENTATION_PLAN.md)과 [문서 안내](../README.md), 실행 확인은 [로컬 체크리스트](AGORA_V1_5_LOCAL_SMOKE_CHECKLIST.md)를 따른다.
