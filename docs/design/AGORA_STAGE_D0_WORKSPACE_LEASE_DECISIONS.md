...
강제하지 않는 대신 **보이게 한다.** 묶음이 끝나면 담당자 폴더 밖에서 바뀐 파일을
찾아 안내로 남긴다(`chat-room.js`의 `reportFolderContractBreaches`). 조용히 덮어써진
파일을 사용자가 한참 뒤에 발견하는 것이 가장 나쁜 결과이기 때문이다.

관측 방식이 `workspace-diff`가 아닌 이유:

```text
workspace-diff  → baseline(HEAD·checkpoint) 이후 전부
                  = 사용자가 미리 갖고 있던 변경까지 담당자 탓으로 돌린다
                  + git이 아닌 작업 폴더에서는 아예 답하지 못한다
workspace-scan  → 묶음 시작 시각 이후에 수정된 파일만
                  = 이번 실행이 바꾼 것만 남는다. git이 없어도 같다.
```

경계 조건은 놓치는 쪽으로 기운다(`mtime > 시작시각`, 등호 없음). 수정시각 해상도가
1초인 파일 시스템에서는 시각이 내림되므로, 등호를 넣으면 실행 직전 사용자 변경이
담당자 탓이 된다. 못 잡는 것보다 엉뚱한 파일을 지목하는 쪽이 나쁘다. 같은 이유로,
훑기가 상한(파일 수·시간)에 걸리면 반쪽짜리 목록을 쓰지 않고 아무 말도 하지 않는다.

**남는 한계**: 동시에 돌았으므로 어떤 변경이 누구 것인지 귀속할 수 없다. 그래서
안내는 "폴더 밖이 바뀌었다"까지만 말하고 담당자를 지목하지 않는다. 남의 폴더 안에
쓴 경우도 구분할 수 없다.
