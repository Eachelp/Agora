...
> 후속 상태 안내(2026-09-06): 초반의 Stage 5~6 ‘후속’ 표기는 최초 착수 범위다. 이후 본문 §8~9에 기록한 Handoff-aware FSM, 역할·팀 상담, `@팀 실행`의 READY 게이트, 선택적 Archivist까지 구현되었다. 역할의 Handoff 요청은 기존 FSM을 보완하며 호출 순서를 자율 결정하는 Professional V2는 포함하지 않는다. 현재 UI는 고정 단계 진행률 대신 현재 작업·대기 사유·다음 행동을 표시하고, 사용자 확인 목록·승인 후 재개·기록 재시도·복원 가능 여부를 제공한다. 모델 목록의 갱신 범위와 현재 사용법은 [문서 안내](../README.md)를 따른다. 아래 테스트 숫자는 해당 구현 당시 기록이며 현재 검증 결과는 [로컬 체크리스트](AGORA_V1_5_LOCAL_SMOKE_CHECKLIST.md)에 따로 남긴다.
...
2차 패스(수정 회귀 + 정적 가상 실행 추적)와 그 잔여분(보안/테스트 품질/
tail 관점)에서 추가로 확정된 것: Archivist 페르소나 누락(그룹 채팅 참가자로
떨어져 "대화에 이어 답하라"와 transcript 차단이 한 프롬프트에서 충돌),
제어 블록 뒤 꼬리 코드펜스가 "빈 줄"로 가려져 end-anchor가 뚫리고 펜스
본문이 화면·TASK.md에서 삭제되던 것(가림 문자를 비어휘·비공백 문자로 교체 —
이제 그 제어는 산문으로 남고 펜스는 보존된다), chat 권한 구현자 상담의
"파일 읽기 가능" 오안내. 테스트 품질 지적에 따라 Handoff 예산 회귀 테스트를
원래 결함(고정 예산 8에서 10 hop 흐름의 Archivist 유실)이 실제로 재현되게
바꾸고, 자유토론 부분 실패·상속 키 Preset·다중 역할 멘션 안내·즉시 거부 시
